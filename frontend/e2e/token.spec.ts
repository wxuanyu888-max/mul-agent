...
test.describe('Token Usage Panel', () => {
  ...
  test('should have refresh button for LLM logs', async ({ page }) => {
    ...
    const refreshButton = page.locator('button').filter({ hasText: '刷新' }).or(page.locator('[aria-label*="refresh" i]'));
  ...
  test('should call token usage API when loading panel', async ({ page }) => {
    ...
    const tokenRequestPromise = page.waitForRequest((request) =>
      request.url().includes('/api/v1/token-usage') && request.method() === 'GET'
    );
  ...
  test('should load LLM logs when clicking refresh', async ({ page }) => {
    ...
    const hasLogsOrLoading = await page.locator('text=LLM 调用').count() > 0 ||
