...
""")
    ...
    def refresh_state_bar():
        """后台刷新状态栏"""
        while not stop_refresh.is_set():
            if state_bar.enabled:
                state_bar.refresh()
            time.sleep(0.5)  # 每0.5秒刷新一次

    # 启动刷新线程
    if not no_state_bar:
        refresh_thread = threading.Thread(target=refresh_state_bar, daemon=True)
    ...
    while True:
        try:
            ...
            try:
                # LLM 正在决定路由
                state_bar.set_state(AgentState.LLM_DECIDING, "LLM 决策中...")
